...
def main():
    ...
    while True:
        ...
        if current_person != "Unknown" and current_person_entered:
            ...
            if time_difference.seconds >= 1:  # Log entry every (set) minutes/seconds
                update_log(current_person_log_path, "Entered the class.")
                if time_difference.seconds >= 3:  # Log entry every 2 minutes
                    if current_person_visible:
                        update_log(current_person_log_path, "Still in class.")
                    else:
                        if time_person_not_visible is not None:
                            if (current_time - time_person_not_visible).seconds >= 4:  # If not visible for 5 minutes
